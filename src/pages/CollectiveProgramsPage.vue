  ...
  <div class="min-h-screen bg-[#edeae6] pb-20">
    ...
    <div class="bg-[#202c27] text-white px-5 py-6">
      ...
      <div class="flex items-center">
        ...
        <div class="ml-4 flex-1">
          <h1 class="text-2xl font-light tracking-wide">Коллективные программы</h1>
          <p class="text-white/70 text-sm mt-1 font-light">Тепло, живой пар, общее действие и единение</p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="px-5 py-5 space-y-4">
      ...
      <div v-if="pageContent" class="bg-[#e3ded3] rounded-xl border border-[#c2a886]/20 p-4">
        <h3 class="font-semibold text-gray-900 mb-3 text-[15px]">О коллективных программах</h3>
        <div class="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{{ pageContent }}</div>
      </div>
      ...
      >
        ...
        <h3 class="text-base font-semibold text-gray-900 mb-2">Коллективные программы</h3>
        <p class="text-sm text-gray-600 leading-relaxed">Скоро здесь появится информация о коллективных программах</p>
      </div>
      ...
      <div v-else class="space-y-3">
        <router-link
          ...
          :key="program.id"
          :to="{ name: 'CollectiveProgramDetail', params: { id: program.id } }"
...
export default {
  ...
  methods: {
    ...
    async loadPageContent() {
      try {
        ...
        const c = this.contentData?.["COLLECTIVE"];
    ...
    async loadCollectivePrograms() {
      try {
        ...
        this.collectivePrograms = Array.isArray(data?.programs)
  ...
  async created() {
    try {
      await Promise.all([this.loadPageContent(), this.loadCollectivePrograms(), this.loadAllBranches()]);
    } catch (e) {
