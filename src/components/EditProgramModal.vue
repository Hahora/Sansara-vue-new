    ...
    >
      ...
      >
        <div class="flex items-center justify-between">
          <h2 class="font-bold text-base">Редактировать программу</h2>
          <button
            @click="$emit('close')"
      ...
      <!-- Форма -->
      <form @submit.prevent="saveProgram" class="p-3 space-y-2">
        <!-- Название -->
        <div>
...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        branch_id: this.program.branch_id || null,
        max_participants: this.program.max_participants || 10,
        is_active: this.program.is_active !== false,
      },
      pricingTiers: JSON.parse(
        JSON.stringify(this.program.pricing_tiers || [])
      ),
      nextTempId: 1,
    };
  },
  ...
  methods: {
    ...
    addPricingTier() {
      // Пытаемся взять длительность из последнего тира, если есть
      const lastDuration =
        this.pricingTiers.length > 0
          ? this.pricingTiers[this.pricingTiers.length - 1].duration_minutes
          : 120;

      this.pricingTiers.push({
        tempId: `new_${this.nextTempId++}`,
        min_guests: 1,
        max_guests: 1,
        price: 0,
        duration_minutes: lastDuration,
        price_type:
    ...
    async saveProgram() {
      try {
        ...
        console.log("Сохранение программы:", this.formData);
        ...
        updateData.pricing_tiers = this.pricingTiers.map((tier) => {
          const tierData = {
          ...
          // Если у тира есть id (не новый), добавляем его
          if (tier.id && !tier.tempId) {
            tierData.id = tier.id;
          }
        ...
        const updatedProgram = await programAPI.adminUpdate(
          this.program.id,
          updateData
        );
        ...
        this.$emit("saved", updatedProgram);
        this.$emit("close");
      } catch (error) {
