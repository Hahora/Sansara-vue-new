    ...
    >
      ...
      >
        <div class="flex items-center justify-between">
          <h3 class="font-bold text-base">Редактировать филиал</h3>
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <form @submit.prevent="updateBranch" class="space-y-3">
          ...
          <div class="bg-gray-50 rounded-lg p-3">
            ...
            <div class="text-xs text-gray-600 space-y-0.5">
              ...
              <div>Создан: {{ formatDate(branch.created_at) }}</div>
              <div>Обновлён: {{ formatDate(branch.updated_at) }}</div>
            </div>
          </div>
      ...
      handler(newVal) {
        if (newVal) {
          this.formData = {
            ...
            phone: newVal.phone || "",
            tg_username: newVal.tg_username || "",
            description: newVal.description || "",
            manager_name: newVal.manager_name || "",
  ...
  methods: {
    ...
    async updateBranch() {
      try {
        ...
        console.log("Обновление филиала:", this.formData);
        ...
        const response = await branchAPI.update(this.branch.id, dataToSend);

        console.log("Филиал обновлен:", response);

        this.$emit("updated");
        this.close();
      } catch (error) {
        console.error("Ошибка при обновлении филиала:", error);
        this.error = error.message || "Не удалось обновить филиал";
      } finally {
